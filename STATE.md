...
> 生成时间：2026-07-21 — Head `d86b190`（ff-only 合并 + P0 安全加固 + 回归修复）+ 本地未推送提交（F2 HQ 只读护栏）— 全量门禁 **27/27 ALL GREEN**（含 1 个重型模型测试默认 SKIP；F1/F2/F6 store 修复 + P5 GUI 工作台 + P2 OCR 适配器 + P3 结构化抽取/实体解析 均已落地回归）

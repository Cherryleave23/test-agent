...
| **F2** | 🔴已修 | `hq_kb`「厂商分发/实例只读」未强制：仅 `enterprise_id=HQ_ENT` 写同表，无 readonly 标志/编辑护栏 | ✅ 已修 `store.py`：HQ 只读由分区（ent=`'hq'` + `part='hq_kb'`）保证，`_row_readonly` 先判 `ent==HQ_ENT`；`delete_corpus`/`update_corpus` 对 HQ 行抛 `ReadonlyError` 拒绝改写/删除（**不向内容型 meta 塞控制标志 `readonly`**，保留 F1 的 meta 纯净语义）；回归 `harness/test_store_hq_readonly.py`（F2a 护栏识别只读 / F2b 删 HQ 拒 / F2c 改 HQ 拒 / F2d 企业 b_kb 可删改 / F2e 只读不影响 F6 跨企业可读，全绿） |
| **F3** | 🔴已修(store) | retrieve 侧未用 `meta.kind` 路由/加权：产品问答 vs 育儿知识 vs 成分机制未分流 | ✅ store 侧已修：① `_index`/`_add_corpus`/`update_corpus` 把 `meta.kind`（product_text/article/ingredient）写入 Chroma metadata；② `retrieve` 新增 `kind_filter`（按内容类型路由）与 `kind_weight`（按 kind 乘性加权），默认 None 完全向后兼容；回归 `harness/test_store_kind_routing.py`（F3a metadata.kind 正确 / F3b 路由排除 / F3c 加权置顶 / F3d 默认向后兼容，全绿）。**后续跨模块**：MOD-agent 检索意图识别 → 选 `kind` 传入 `retrieve`（store 能力已就绪，待 agent 侧接线） |
| **F4** | 🔴已修 | bundle 运输 + 触发未定义：此前无把 dataproc 产出 bundle 灌入 store 的加载器，且「谁触发」未定义 | ✅ 已修：新建 `src/ingest/importer.py`——① `load_bundle` 按 products→hq_products→corpus 顺序加载，结构化产品先建拿 pid、product_text 语料按 `product_uid→product_id` 绑定（F1 契约），单条失败不中断、记入 errors；② `scan_and_load` 扫收件箱、成功移 `processed/`、失败移 `failed/` 留痕，移动即幂等；③ `load_on_startup` 读 `BUNDLE_INBOX_DIR` 启动自动加载（未配置 no-op，已接入 `app.build_instance`）。回归 `harness/test_ingest_bundle_load.py`（F4a 落库 / F4b uid→pid 绑定 / F4c HQ 商品播种 / F4d 幂等 / F4e 不匹配拒收 / F4f 无 manifest 跳过，全绿）。**后续**：微信管理指令调 `scan_and_load` 手动触发（机制已就绪，待 MOD-wechat 接线） |
| **F5** | 🔴已修(store) | `pending` 商品确认 UX 未定义：resolver 产 `status=pending`，员工在 agent 端确认/合并/删除流未闭环 | ✅ store 数据侧已修：新增 `list_pending_products`（奶粉按 reg_number 空 / 营养品按 health_license 空 判定 pending）、`confirm_product`（写对应批准号不再 pending）、`delete_product`（删商品 + 连带 b_milk/b_nutrition 语料分块与 Chroma 向量）；回归 `harness/test_store_pending_product.py`（F5a 只列 pending / F5b 确认后出列 / F5c 删除连带语料，全绿）。**后续**：微信侧「待确认列表展示 + 确认/合并/删除」UX 流（调上述原语）待 MOD-wechat 接线 |
| **F6** | 🔴已修 | `retrieve` 第 4 步回查 `if enterprise_id is not None and != 运行实例: continue` 把 `hq_kb` 行（ent=`"hq"`）**全部丢弃**，导致 HQ 共享库跨企业实际不可读，Chroma `where` 联合 `HQ_ENT` 成死代码 | ✅ 已修 `store.py`（**方案②**：回查条件改为 `ent not in (None, HQ_ENT) and ent != enterprise_id` 才丢弃，HQ 对所有企业可见；同步把模块 docstring 的 `enterprise_id IS NULL` 对齐为实际 `enterprise_id='hq'`）+ 回归 `harness/test_store_hq_retrieve.py`（F6a 本企业读 HQ / F6b 异企业读 HQ / F6c 异企业不读他企 b_kb / F6d HQ ingredient 可读且 kind 存活，全绿）。F2 只读护栏已另补（见 F2 行） |

...
### P2-v2 终极实战 harness（2026-07-21）

> 来源：用户要求对 agent 综合复杂问题能力进行终极压测。模拟真实门店场景：5 个差异化宝宝
> （含早产/多重过敏/极简/中等）的 34 条碎片化信息随机打乱逐条归档，再以随机顺序交叉提问。

**测试设计：**
- 5 个宝宝 ground truth：A=共青二宝（复杂：早产35周+病史+喂养史）/ B=悦悦（简单：鸡蛋过敏）/ C=小石头（极复杂：早产32周+牛奶蛋白+大豆过敏+NICU+脑室出血+贫血）/ D=朵朵（极简：仅月龄+段位）/ E=辰辰（中等：海鲜过敏+预算400）
- 34 条碎片化信息：每条 = 一个事实（出生日期/月龄/孕周/体重/住院/品牌/段位/过敏/预算/客户名），标注归属宝宝
- 随机打乱（`random.Random(42)`）后逐条发送给 `resolve_and_archive`，模拟 CRM 导入后对话中补充属性
- 5 个问题（奶粉建议/辅食建议/维生素/营养补充/过敏管理）随机分配给 5 个宝宝、随机顺序提问
- MockProvider 模拟完美 LLM：被调用时返回该宝宝全部 ground truth 属性

**harness 验收（`harness/test_ultimate_baby_harness.py`，`@module baby`）：**
| 编号 | 验收点 | 结果 |
|------|--------|------|
| P30 | 5 个宝宝档案存在且无重复、全 confirmed | PASS |
| P31-P35 | 各宝宝基础属性（baby_age/stage/allergens/category/budget）与 ground truth 匹配 | PASS |
| P36 | 无跨宝宝串档（A 的过敏原/病史/品牌不出现在 B 的档案中） | PASS |
| P37 | 5 个回答非空（>10 字符） | PASS |
| P38 | 每个回答引用正确的宝宝名 | PASS |
| P39 | 焦点切换正确（提问后焦点指向对应宝宝） | PASS |

**诊断指标（info 级，不计 PASS/FAIL）：**
- 结构化字段覆盖率：100%（5 个有结构化字段的宝宝全部匹配）
- LLM 调用率：89%（35/39 条消息经 LLM 路径，10% 经 `focus_is_stable` 规则抽取短路）
- 每个宝宝 LLM 均被调用 ≥1 次，确保结构化字段完整抽取

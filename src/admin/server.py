...
安全设计：
  ...
  - YAML 路径验证：仅允许写入 deploy/ 目录下的配置文件
...
def _get_admin_token() -> str:
    """获取 admin token：环境变量设置时启用认证，未设置时放行（开发模式）。"""
    token = os.environ.get("AGENT_ADMIN_TOKEN", "").strip()
    return token  # 空串表示开发模式，不启用认证
...
def create_app(cfg: EnterpriseConfig) -> FastAPI:
    app = FastAPI(title="母婴 Agent 管理后台", version="0.2.0")
    ...
    def _get_store() -> KnowledgeStore:
        if _store_holder["store"] is None:
            # admin 后台不需要 reranker；用 "none" 避免 mock 不支持
